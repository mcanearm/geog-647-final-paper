...
abstract: >
  ...
  busy historical trolley routes remain the busiest routes of today. While reimplementation of the 1917 
  ...
  particular effectiveness of the network. There is no evidence that the transit
  networks of the past would be more effective in today's cities than the existing networks,
...
but with some differences regarding the spatial distribution of the system.
Milwaukee's streetcar history differed from comparable cities
...
trolleys. This guide us an empirically based assessment of the past network by today's
standards to better understand the growth of cities and how future light rail projects
should can be planned.
...
were loaded into QGis for easier manipulation. A shapefile of bus routes in Milwaukee was loaded into 
...
```{r populationService, fig.height = 4.5, fig.cap = "Current bus routes and 1917 trolley routes of Milwaukee laid over 2010 census blocks. All census blocks in the Milwaukee CSA are considered, but this set is truncated for legibility.", results='asis', fig.pos='here'}
...
```{r bus_trolley_plot, echo = FALSE, fig.height=7, fig.cap="Map of the 4 counties comprising the Milwaukee MSA. The majority of the MSA by land area is not served by public transit into Milwaukee proper. The 1917 trolley network serves Milwaukee proper, while the buses serve only the outer city and inner suburbs.", fig.pos='here', message=FALSE}
...
the role of historical street routes in Hartford, Connecticut. In that study, trolley lines
were less dense than the newer bus networks, but the bus system's lower headways
and smaller number of vehicles resulted in a lower overall level of service 
from buses despite the larger network[@{polinski_transit_2015]. This highlights the role
of the route management in providing adequate service - total distance and route-miles
is not the single determinant of service level.

In terms of the physical routes involved, the 21st 
...
that lives within a census block. 
...
The most important
transit routes have also remained relatively constant from 1917 until 2010. The bus routes 
which serve the highest numbers of people overlap or run near the
trolley network, though they now extend much further from the downtown area. 
Given that many of the routes are the same or similar and the old network continues
to serve the densest part of Milwaukee, what would be the effect of replacing
the new bus routes with the trolley network? 

```{r busRouteOverlap, fig.width = 3.0, fig.height = 3.5, fig.cap='The 20 bus routes serving the highest number of people (adjusted) overlaid over the 1917 trolley network. Most of the bus routes follow along the same corridors as trolley routes.', fig.pos='here'}
...
total population within 1km of the line would be reduced by `r sprintf('%0.0f', ((transportSummary[transport == 'Bus', Population] - transportSummary[transport == 'Trolley', Population])/transportSummary[transport == 'Bus', Population])*100)`. Transit is a 
trade-off between massification and atomization, and the effect of a light rail 
...
rail[@william_t_king_economic_2014][@mallett_federal_nodate].  
Without the underlying conditions that support mass transit, it is not abundantly 
clear where the new economic benefits will come from or why a shorter light rail network
limited to the downtown area would be more would be more effective

Conversely, in another study of a light rail extension in Salt Lake City, there
was an estimated 670 percent increase in ridership over the previous bus line it 
replaced[@werner_evaluating_2016]. Another study of light rail usage found
...
In 2019, buses provided about 29 million rides[@noauthor_2019_nodate].
Let's assume these rides were restricted to census blocks and adjusted population calculated
previously to estimate a number of rides per person. This yields an average ridership
of `r sprintf('%0.01f', ridesPerPerson['Bus'])` per person per year. To match the total number of rides to 
compensate for the lost riders of the the bus, the new "trolley-only" riders would
...
further need to come from people who are more likely to be within walking distance of
destinations already.

The increase is on the high end of estimates from the literature, though
not the highest. In the cases of Tuscon and Minneapolis, transit successes
were also been accompanied by increased residential and mixed-use development
within the corridors of interest[@william_t_king_economic_2014][@cao_influence_2014]. 
This speaks to the importance of urban layout in the past as establishing streetcars
as enablers of the urban activity, and not the causes of it. The cities of 1917 were denser 
than the cities of 2010. Kenneth Jackson put it best regarding the era of the walking city:
...
more dense than the Milwaukee of the mid 20th century, and it appears that this
is more important than the type of transit provided.

Ultimately, the benefits of light rail over a bus line are fairly limited until
the surrounding environment is allowed to change to support it. In some cases,
new light rail projects succeed in dramatically increasing property values
around the line[@william_t_king_economic_2014] - this an example of a demand side shift, 
since more people want to live near the transportation benefits. Light rail also
enables large supply side shifts in housing, but only if zoning laws are changed to enable
more housing, work, and retail destinations near the lines. 
...
project, it is primarily seen seen as driver of economic development. The Hop's route 
...
spatial organization and activities more than what transit option is provided. 
Even if Milwaukee's trolleys had never been ripped up and replaced with buses, 
their routes would still be connecting areas of decreasing population in the downtown core. 
Thus, there appears to be little evidence that the replacement of bus routes with 
trolleys would offer any genuinely new benefits *et ceteris paribus.*
...
displacing long-term residents. In its current form, The Hop mostly accrues
benefits to just the downtown core and private entities therein, and the residential
neighborhoods served by the line tend to be more affluent than average[@diciaula_transit-oriented_2019]. 
In our hypothetical transit network, we may be super-charging gentrification effects in 
the short-term. In the long-term, it could be that an enormous increase in the amount and 
types of housing avaiable, including smaller than currently allowable homes and apartments, could
generate new affordable housing options through a supply-side shock. 
For a displaced resident asking "will I still be able to live here?", 
...
percent of the Milwaukee CSA population. Our hypothetical light rail system, by contrast,
...
This would leave `r prettyNum(round(sum(milwaukee_blocks$pop10) - sum(bothBlocks$pop10_adj), -3), big.mark=',')`
people in the Milwaukee MSA without no access to public transit at all. The outcome
of this would be a large number of Milwaukee transit riders unable to get to their jobs,
and perhaps a percentage of them now driving cars into the city, undoing some of the
anti-congestion benefits of the light rail. And considering the number of people no 
longer served by buses, the number of new drivers could be substantial.

# Conclusion

Urban planners often cite the need to reduce automobile traffic through
transit utilization. Light rail propositions are new yet altogether familiar
piece of technology, at once both a sign of progress and a callback to 
an earlier time of growing cities.

If Milwaukee is any indication, many urban problems of congestion and pollution
can be solved by looking to the past for inspiration.
The city, like most American cities, has changed too much since 1917 to re-use
the historical trolley network without other policies in place. Implementing such 
a network would result in `r sprintf('%0.0f', ((transportSummary[transport == 'Bus', Population] - transportSummary[transport == 'Trolley', Population])/transportSummary[transport == 'Bus', Population])*100)` 
...
the major changes would need to be enacted in order to get increase light 
...
However, recent research has shown that implementing light rail along with TOD policies
can generate large economic and community benefits, reducing congestion and increasing
walkability. But to implement our historical trolley network in the name of 
revitalizing downtown and strengthening communities would be callously destructive 
in its own way, with no guarantee that the proposed benefits would outweigh the economic
consequences.
